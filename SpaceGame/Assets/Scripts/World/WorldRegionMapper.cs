...
// Maps ISO 3166-1 alpha-3 country codes to RegionRegistry region indices.
// Index order matches RegionRegistry.s_regionIds exactly.
public static class WorldRegionMapper
{
    static readonly Dictionary<string, byte> s_map = new()
    {
        // 0 — north_america
        {"USA",0},{"CAN",0},{"GRL",0},
        // 1 — c_america
    ...
    {
        if (!string.IsNullOrEmpty(iso3) && s_map.TryGetValue(iso3, out index))
            return true;
        index = 255;
        return false;
    }
